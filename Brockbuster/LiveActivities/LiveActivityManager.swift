...
final class LiveActivityManager {
    ...
    func startIfNeeded(itemId: String, posterURL: URL?, title: String, subtitle: String?, isPlaying: Bool, positionSeconds: Double, durationSeconds: Double) {
        ...
        if let activity, activity.attributes.itemId == itemId {
            update(title: title, subtitle: subtitle, isPlaying: isPlaying, positionSeconds: positionSeconds, durationSeconds: durationSeconds)
            return
        }
    ...
    func update(title: String, subtitle: String?, isPlaying: Bool, positionSeconds: Double, durationSeconds: Double) {
        guard #available(iOS 16.1, *) else { return }
